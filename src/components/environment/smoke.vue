<template>
  <div class="main">
    ...
    <div class="query">
      ...
      <div class="select">
        <el-select v-model="smoke_id" class="m-2" placeholder="烟雾#1" size="large">
          <el-option
              v-for="item in smoke_id_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
          />
    ...
    <div class="chart">
      <span>单位：mg/m³</span>
      <div class="chartPlace" id="smokechart">
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    initchart(){
      var smokechart = echarts.init(document.getElementById('smokechart'));
      var smokeOption =  {
        legend: {
          data: ['烟雾浓度']
        },
        ...
        series: [
          {
            name:'烟雾浓度',
            data: [3.5, 4.1, 2.9, 1.1, 6.6, 5.3, 3.3,2.2, 5.1, 4.4],
            type: 'line'
          }
        ]
      };
