<template>
  <div class="main">
    ...
    <div class="query">
      ...
      <div class="select">
        <el-select v-model="disp_id" class="m-2" placeholder="位移测量#1" size="large">
          <el-option
              v-for="item in disp_id_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
          />
    ...
    <div class="chart">
      <span>单位：mm</span>
      <div class="chartPlace" id="dispchart">
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    initchart(){
      var dispchart = echarts.init(document.getElementById('dispchart'));
      var dispOption =  {
        legend: {
          data: ['X方向位移','Y方向位移','H方向位移']
        },
        xAxis: {
          type: 'category',
        ...
        series: [
          {
            name:'X方向位移',
            data: [12.5, 13.1, 16.9, 12.1, 11.6, 14.3, 15.3,11.2, 11.1, 16.4],
            type: 'line'
          },
          {
            name:'Y方向位移',
            data: [7.5, 8.1, 9.9, 8.1, 7.6, 9.3, 9.3, 7.2, 6.1, 8.4],
            type: 'line'
          },
          {
            name:'H方向位移',
      ...
      dispchart.setOption(dispOption);
    }
  },
  mounted() {
    this.initchart()
