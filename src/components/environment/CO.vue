<template>
  <div class="main">
    ...
    <div class="query">
      ...
      <div class="select">
        <el-select v-model="CO_id" class="m-2" placeholder="一氧化碳#1" size="large">
          <el-option
              v-for="item in CO_id_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
          />
    ...
    <div class="chart">
      <span>单位：mg/m³</span>
      <div class="chartPlace" id="COchart">
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    initchart(){
      var COchart = echarts.init(document.getElementById('COchart'));
      var COOption =  {
        legend: {
          data: ['一氧化碳浓度']
        },
        ...
        series: [
          {
            name:'一氧化碳浓度',
            data: [12.5, 23.1, 16.9, 17.1, 21.6, 29.3, 19.3,18.2, 23.1, 20.4],
            type: 'line'
          }
        ]
      };
