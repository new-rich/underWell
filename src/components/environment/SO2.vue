<template>
  <div class="main">
    ...
    <div class="query">
      ...
      <div class="select">
        <el-select v-model="SO2_id" class="m-2" placeholder="二氧化硫#1" size="large">
          <el-option
              v-for="item in SO2_id_list"
              :key="item.value"
              :label="item.label"
              :value="item.value"
          />
    ...
    <div class="chart">
      <span>单位：mg/m³</span>
      <div class="chartPlace" id="SO2chart">
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    initchart(){
      var SO2chart = echarts.init(document.getElementById('SO2chart'));
      var SO2Option =  {
        legend: {
          data: ['二氧化硫浓度']
        },
        ...
        series: [
          {
            name:'二氧化硫浓度',
            data: [1.2, 1.1, 1.9, 1.1, 2.6, 2.3, 1.3,1.2, 2.1, 2.4],
            type: 'line'
          }
        ]
      };
      SO2chart.setOption(SO2Option);
    }
  },
  mounted() {
    this.initchart()
